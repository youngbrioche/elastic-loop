...
One honest difference, before the parallel runs away with us. A factory and a loop are not the same kind of problem. The ten-thousandth car door has a known target, and the whole apparatus exists to drive variation toward zero: every door identical, every deviation a defect. A loop sits in the other kind of problem, where the right answer is not known up front and only shows itself once you have tried a few. So the goal inverts. The factory fights to eliminate variance; the loop exists to harvest it, generating several real variants and keeping the one that holds. The machinery for running a variable process without hand-inspecting every part carries over. The war on variation does not, because here the variation is the product, and backpressure is the wall that keeps it inside useful bounds rather than the press that stamps it flat.
