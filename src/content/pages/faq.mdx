...
Lower on the list. The measurements on [Why](/why) and [Harness](/harness) hold the model fixed and swap only the scaffold around it, the <Term id="harness">harness</Term>, and the score still swings hard: the harness alone can double it. Pick a capable frontier model, then put the argument where the leverage sits: the loop, the backpressure, and the grading around it.
