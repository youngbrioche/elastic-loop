...
## What about feedback that only arrives slowly?

Some of the signals that matter most are also the slowest: whether a feature actually moved the business, whether users adopt it, whether the architecture still holds after a year of change. They grade you quarters after the work shipped. A loose loop cannot run honestly against a grader that slow. The agent would work for hours, a fleet for days, against a verdict that will not arrive in time to steer anything.

So the latency of your tightest trustworthy grader sets the ceiling on how far you can let go. If the only real signal is the market months out, the loop stays tighter than you might like, and no stronger model changes that, because nothing can close the loop fast enough.

The way out is the move a baker makes. The real test of a cake is the table tonight, but the experienced baker does not wait for it. A toothpick that comes out clean, the smell, the spring-back when you press the top, these are fast proxies that track the slow truth and let you correct now. In a loop those proxies are the unglamorous grading artifacts, the scenarios and counterexamples and leading-indicator metrics that predict what the slow signal will eventually say. Building them is how you buy back loop size against a slow world. Until they exist, a slow grader is an honest reason to keep the loop short.
