...
Because tactics age in months and strategy doesn't. Anything I could pin down about today's exact tools, the flags, the model names, the way one harness wires up its hooks, would be wrong by the time you read it, and the book is meant to outlive that. So it stays at the level that holds: how to size a loop, what context buys you, why the judgment moved to the ends of the loop. The book lives and grows, but it grows along that line, not by chasing every new release into a manual.
